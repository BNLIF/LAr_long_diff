...
void fit_test(Int_t num = 0, Int_t flag = 0, Int_t fit_min = 200, Int_t fit_max = 6000){
  ...
  for (Int_t i=0;i!=h1->GetNbinsX()-1;i++){
    Double_t prev_content = h1->GetBinContent(i+1);
    Double_t content = h1->GetBinContent(i+1);
    Double_t diff = content - prev_content;
    h2->Fill(diff);
  }
  ...
  for (Int_t i=0;i!=h1->GetNbinsX()-1;i++){
    Double_t prev_content = h1->GetBinContent(1);
    Double_t content = h1->GetBinContent(i+1);
    Double_t diff = content - prev_content;
    h3->Fill(diff);
  }
  ...
  if (flag==1){
    ...
    cout << num << " " << fit_min << " " << fit_max << " " << f1->GetParameter(4) << " " << f1->GetParError(4) << " " << f1->GetChisquare()/f1->GetNDF() << endl;
  }else{
